...
namespace VHS
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        #region BuiltIn Methods     
            ...
            {
                if(m_yawTransform != null && !GameManager.Instance._isVideoPlaying && !GameManager.Instance._isCutscenePlaying)
                    RotateTowardsCamera();

                if(m_characterController && !GameManager.Instance._isVideoPlaying && !GameManager.Instance._isCutscenePlaying)
                {
                ...
                protected virtual void HandleHeadBob()
                {
                    ...
                    if(movementInputData.HasInput && m_isGrounded  && !m_hitWall)
                    {
                        if(!m_duringCrouchAnimation) // we want to make our head bob only if we are moving and not during crouch routine
                        {
                            ...
                            m_headBob.ScrollHeadBob(true,movementInputData.IsCrouching, movementInputData.InputVector);
